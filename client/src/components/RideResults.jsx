  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {trips.map((trip) => {
        ...
        const isCancelled = trip.reservationStatus === "cancelled";

        return (
          <Link
            key={trip._id}
            to={`/trips/${trip._id}`}
            className={`block border rounded-lg shadow hover:shadow-lg transition-all overflow-hidden group ${
              isCancelled ? "opacity-60 grayscale" : "bg-white"
            }`}
          >
            ...
            <div className="p-4 space-y-2">
              ...
              <div className="flex gap-2 flex-wrap text-sm">
                ...
                {isUrgent && (
                  <span
                    ...
                    data-tooltip-id="urgentTip"
                    data-tooltip-content="Trip starts within 24h"
              ...
              <p className="text-sm text-gray-600">
                ...
                >
                  {isCancelled ? "❌ Cancelled" : trip.status || "unknown"}
                </span>
              </p>
              ...
              {!user ? (
                ...
                <p className="text-center mt-2 text-sm text-red-500 font-semibold">
                  🚫 Reservation Cancelled
                </p>
              ) : onCancel && hasReserved ? (
                <button
                  onClick={(e) => {
                    e.preventDefault();
